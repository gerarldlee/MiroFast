...
def create_app(config_class=Config):
    ...
    if should_log_startup:
        logger.info("=" * 50)
        logger.info("MiroFast Backend starting (Laya decision engine)...")
        logger.info("=" * 50)
    ...
    @app.route('/health')
    def health():
        return {'status': 'ok', 'service': 'MiroFast Backend', 'engine': 'laya'}

    if should_log_startup:
        logger.info("MiroFast Backend startup complete")
