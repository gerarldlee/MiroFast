...
class ReportAgent:
    ...
    def __init__(
        ...
        # MiroFast: Laya plans the tool sequence per section (single forward
        # passes) so the LLM only writes prose. Disable to restore the full
        # LLM-driven ReACT loop.
        self.plan_tools_with_laya = os.environ.get(
            'MIROFAST_PLAN_TOOLS', 'true').lower() in ('1', 'true', 'yes')
    ...
    def _get_laya(self):
        """Lazily build the shared Laya decision engine."""
        if self._laya_engine is None:
            from ..decisions import LayaEngine
            self._laya_engine = LayaEngine()
        return self._laya_engine
    ...
    # -- MiroFast: Laya-planned tool execution ------------------------------

    _TOOL_PLAN_SUMMARIES = {
        "insight_forge": (
            "deep insight retrieval: decomposes the question into sub-queries "
            "and assembles entities, relationships and fact chains from the "
            "knowledge graph"
        ),
        "panorama_search": (
            "broad panoramic view of the graph: current valid facts versus "
            "historical or expired ones"
        ),
        "quick_search": (
            "lightweight fast keyword retrieval from the graph"
        ),
        "interview_agents": (
            "live interviews of simulated agents on Twitter and Reddit to "
            "capture their current views and reasoning"
        ),
    }

    def _plan_tools_laya(self, section, outline) -> List[str]:
        """Pick the tool sequence for a section with single Laya passes."""
        engine = self._get_laya()
        state = {
        ...
        for _ in range(3):
            candidates = {
                name: desc
                for name, desc in self._TOOL_PLAN_SUMMARIES.items()
                if name not in used
            }
            candidates["done"] = (
                "enough evidence has been gathered; write the section now"
            )
            choice, _confidence = engine.choice(
                state,
                "Which evidence tool should be called next for this report "
                "section?",
                candidates,
            )
            if choice == "done" or choice not in self._TOOL_PLAN_SUMMARIES:
                break
            used.append(choice)
        if not used:
            used = ["insight_forge", "quick_search"]
        return used

    def _default_tool_parameters(self, tool_name: str, section) -> Dict[str, Any]:
        """Deterministic tool parameters derived from the section context."""
        query = f"{section.title} \u2014 {self.simulation_requirement[:200]}"
        if tool_name == "insight_forge":
            return {"query": query, "report_context": query}
        ...
        if tool_name == "interview_agents":
            return {"interview_topic": section.title, "max_agents": 5}
        return {}

    def _generate_section_planned(
        self,
        section,
        outline,
        ...
        """Laya plans the tools, tools run without LLM turns, then one final
        LLM call writes the section (~2 LLM calls instead of ~6)."""
        report_context = (
            f"Section Title: {section.title}\n"
            f"Simulation Requirement: {self.simulation_requirement}"
        )

        plan = self._plan_tools_laya(section, outline)
        logger.info(f"Section '{section.title}' Laya tool plan: {plan}")
        ...
        for tool_name in plan:
            if progress_callback:
                progress_callback(
                    "generating",
                    int((tool_calls_count / max(len(plan), 1)) * 100),
                    f"Gathering evidence ({tool_calls_count + 1}/{len(plan)})",
                )
            parameters = self._default_tool_parameters(tool_name, section)
            ...
            try:
                result = self._execute_tool(
                    tool_name, parameters, report_context=report_context
                )
            except Exception as exc:
                logger.warning(f"Planned tool {tool_name} failed: {exc}")
                continue
            tool_calls_count += 1
        ...
        if progress_callback:
            progress_callback("generating", 90, "Writing section content...")
        ...
        if response is None:
            final_answer = (
                "(This section generation failed: LLM returned empty "
                "response, please retry later)"
            )
        ...
        logger.info(
            f"Section {section.title} generation completed "
            f"(planned tools: {tool_calls_count})"
        )
        if self.report_logger:
    ...
    def _generate_section_react(
        ...
        if self.plan_tools_with_laya:
            try:
                return self._generate_section_planned(
                    section, outline, previous_sections,
                    progress_callback, section_index,
                )
            except Exception as e:
                logger.warning(
                    f"Laya planned section generation failed ({e}); "
                    "falling back to ReACT loop"
                )
