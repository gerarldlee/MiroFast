...
Return the sub-questions as a JSON list."""
    ...
    def _select_agents_for_interview(
        ...
        MiroFast: ONE batched Laya forward pass ranks every candidate
        profile by interview value (replacing the LLM selection call).
        Falls back to the LLM selection when Laya is unavailable.
        """

        # ── Laya path: batched calibrated ranking ──
        try:
            if getattr(self, '_laya_engine', None) is None:
                from ..decisions import LayaEngine
                self._laya_engine = LayaEngine()
            engine = self._laya_engine
            ...
            instructions = (
                "Would interviewing this simulated user provide valuable, "
                "unique or diverse perspective on the interview topic? "
                "Consider professional relevance to the topic and "
                "viewpoint diversity."
            )
            probs = engine.noul_batch(states, instructions)
            ranked = sorted(
                range(len(profiles)), key=lambda i: probs[i], reverse=True
            )
            top = [i for i in ranked[:max_agents] if probs[i] > 0.35]

            if not top:
                top = ranked[:max_agents]

            selected_agents = [profiles[i] for i in top]
            reasoning = (
                f"Laya ranked {len(profiles)} candidates by interview value "
                f"(top confidence: {probs[top[0]]:.2f})" if top
                else "No candidates"
            )
            return selected_agents, top, reasoning

        except Exception as e:
            logger.warning(f"Laya agent selection failed, falling back to LLM selection: {e}")
