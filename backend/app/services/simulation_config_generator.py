"""
Simulation Configuration Intelligent Generator (MiroFast hybrid)
Time and event configuration via LLM; agent activity schedules via rules;
per-entity stance and sentiment via ONE batched Laya forward pass.

Adopt step-by-step generation strategy:
1. Generate time configuration and event configuration (LLM, 2 calls)
2. Generate agent configurations in batches (Laya stance/sentiment + rules,
   zero LLM; set USE_LLM_FOR_AGENT_CONFIGS=1 for the original LLM path)
...
{{
    ...
    def _get_laya_engine(self):
        """Lazily build the shared Laya decision engine (Flask process)."""
    ...
    def _generate_agent_configs_batch(
        ...
        MiroFast: schedule parameters follow deterministic rules and stance /
        sentiment_bias come from ONE batched Laya forward pass, replacing the
        per-batch LLM call. Set USE_LLM_FOR_AGENT_CONFIGS=1 to restore the
        original LLM-generated configuration.
        """

        use_llm = os.environ.get(
            'USE_LLM_FOR_AGENT_CONFIGS', 'false').lower() in ('1', 'true', 'yes')
...
{{
    "agent_configs": [
        ...
        # MiroFast: one batched Laya pass scores stance + sentiment for the
        # whole batch in a single forward pass (no LLM call).
        laya_scores: List[Dict[str, float]] = []
        if not llm_configs and entities:
            try:
                engine = self._get_laya_engine()
                summary_len = self.AGENT_SUMMARY_LENGTH
                states = [
                    {
                        "entity_name": e.name,
                        "entity_type": e.get_entity_type() or "Unknown",
                        "entity_summary": (e.summary or "")[:summary_len],
                        "simulation_topic": simulation_requirement[:400],
                    }
                    for e in entities
                ]
                laya_scores = engine.stance_sentiment_batch(
                    states, simulation_requirement[:200]
                )
            except Exception as e:
                logger.warning(f"Laya stance/sentiment scoring failed: {e}; falling back to neutral")
                laya_scores = []
        ...
        for i, entity in enumerate(entities):
            ...
            # Override stance / sentiment with the Laya decision when present
            if laya_scores and i < len(laya_scores):
                stance_score = laya_scores[i].get("stance", 2.0)   # 0..4
                sentiment_score = laya_scores[i].get("sentiment", 2.0)
                bias = max(-1.0, min(1.0, (sentiment_score - 2.0) / 2.0))
                if stance_score <= 1.25:
                    stance_label = "opposing"
                elif stance_score >= 2.75:
                    stance_label = "supportive"
                else:
                    stance_label = "neutral"
                # Institutions/media that stay neutral act as observers
                if stance_label == "neutral" and cfg.get("stance") == "observer":
                    stance_label = "observer"
                cfg["stance"] = stance_label
                cfg["sentiment_bias"] = round(bias, 3)
