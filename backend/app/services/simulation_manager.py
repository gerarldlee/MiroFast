...
class SimulationManager:
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "commands": {
                "twitter": f"python {scripts_dir}/run_parallel_simulation.py --twitter-only --config {config_path}",
                "reddit": f"python {scripts_dir}/run_parallel_simulation.py --reddit-only --config {config_path}",
                "parallel": f"python {scripts_dir}/run_parallel_simulation.py --config {config_path}",
            },
            "instructions": (
                f"1. Activate your Python environment\n"
                f"2. Run simulation (scripts located in {scripts_dir}):\n"
                f"   - Run Twitter alone: python {scripts_dir}/run_parallel_simulation.py --twitter-only --config {config_path}\n"
                f"   - Run Reddit alone: python {scripts_dir}/run_parallel_simulation.py --reddit-only --config {config_path}\n"
