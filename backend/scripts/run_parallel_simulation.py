"""
OASIS dual-platform parallel simulation preset script — MiroFast hybrid engine
Run Twitter and Reddit simulations simultaneously with the same configuration file

MiroFast replaces the per-agent LLM tool-calling step with a hybrid policy:
Laya (non-autoregressive decision engine) classifies every active agent's
action, target post and stance in ONE batched forward pass per round, and the
LLM is only called to write post/comment/quote content. Direct actions
(like / dislike / repost / do_nothing) execute with zero LLM calls.

Features:
- Dual-platform (Twitter + Reddit) parallel simulation
- Hybrid round loop: Laya decisions + LLM-only-for-prose generation
- Per-round decision log (decisions.jsonl) with confidence and stance
- Content moderation gate on generated text
...
Log structure:
    ...
    │   └── decisions.jsonl  # Laya decision log (confidence, stance)
    ├── reddit/
...
# ============================================================
# MiroFast hybrid engine: Laya decides (System 1), LLM only writes
# ============================================================
from app.decisions import LayaEngine, run_hybrid_round
from app.llm.gateway import GenerationGateway

_laya_engine = None
_generation_gateway = None

_MODERATE_CONTENT = os.environ.get(
    'LAYA_MODERATE_CONTENT', 'true').lower() in ('1', 'true', 'yes')
_LLM_FALLBACK = os.environ.get(
    'LAYA_LLM_FALLBACK', 'false').lower() in ('1', 'true', 'yes')


def get_shared_laya_engine() -> LayaEngine:
    """One LayaEngine (and one Router) per simulation process."""
    global _laya_engine
    if _laya_engine is None:
        _laya_engine = LayaEngine()
    return _laya_engine


def get_shared_generation_gateway() -> GenerationGateway:
    """One generation gateway for simulated content."""
    global _generation_gateway
    if _generation_gateway is None:
        _generation_gateway = GenerationGateway(purpose="posts")
    return _generation_gateway
...
async def run_twitter_simulation(
    ...
    for round_num in range(total_rounds):
        ...
        # MiroFast hybrid round: Laya decides the crowd in one batched
        # forward pass; the LLM only writes post/comment/quote content.
        round_stats = await run_hybrid_round(
            env=result.env,
            active_agents=active_agents,
            engine=get_shared_laya_engine(),
            gateway=get_shared_generation_gateway(),
            platform="twitter",
            round_num=round_num + 1,
            simulated_hour=simulated_hour,
            decision_log_path=os.path.join(simulation_dir, "twitter", "decisions.jsonl"),
            moderate=_MODERATE_CONTENT,
        ...
        if (round_num + 1) % 20 == 0 or round_num + 1 == total_rounds:
            progress = (round_num + 1) / total_rounds * 100
            log_info(f"Day {simulated_day}, {simulated_hour:02d}:00 - Round {round_num + 1}/{total_rounds} ({progress:.1f}%) "
                     f"[decisions: {round_stats.decision_ms:.0f}ms, generation: {round_stats.generation_ms:.0f}ms, "
                     f"manual: {round_stats.manual_count}, generated: {round_stats.generated_count}]")
...
async def run_reddit_simulation(
    ...
    for round_num in range(total_rounds):
        ...
        # MiroFast hybrid round: Laya decides the crowd in one batched
        # forward pass; the LLM only writes post/comment content.
        round_stats = await run_hybrid_round(
            env=result.env,
            active_agents=active_agents,
            engine=get_shared_laya_engine(),
            ...
            round_num=round_num + 1,
            simulated_hour=simulated_hour,
            decision_log_path=os.path.join(simulation_dir, "reddit", "decisions.jsonl"),
            moderate=_MODERATE_CONTENT,
            llm_fallback=_LLM_FALLBACK,
            log=log_info,
        )
...
async def main():
    ...
    # Warm up the Laya decision engine (loads the checkpoint once; the
    # hybrid round loop cannot run without it).
    try:
        laya_engine = get_shared_laya_engine()
        log_manager.info(f"Warming up Laya decision engine (model={laya_engine.model}, device={os.environ.get('LAYA_DEVICE', 'auto')})...")
        laya_engine.warmup()
        log_manager.info("Laya decision engine ready")
        log_manager.info(f"Hybrid policy: moderate_content={_MODERATE_CONTENT}, llm_fallback={_LLM_FALLBACK}, "
                         f"confidence_threshold={laya_engine.confidence_threshold}")
    except Exception as e:
        log_manager.info(f"FATAL: Laya engine failed to load: {e}")
        print(f"FATAL: Laya engine failed to load: {e}")
        sys.exit(1)
